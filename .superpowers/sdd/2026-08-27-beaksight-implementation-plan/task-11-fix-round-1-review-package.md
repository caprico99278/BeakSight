# Task 11 fix round 1 independent re-review — Git-free SDD

Status: fixed-scope read-only re-review. Git operations are prohibited.

## Review instructions

Verify all 30 SHA-256 entries. Read the first specification review, `task-11-fix-round-1-brief.md`, current report/progress, and manifested source/tests/fixtures.

Adjudicate all entering findings:

1. Critical exact-node binding: detached rediscovery, recollected facts, admission, click, and post-condition must use one retained ElementHandle; DOM insertion/reorder must never cause another unsafe node to be clicked.
2. Important deadline: locator/ordinal/handle/fact boundaries and click dispatch must be separately absolute-deadline-gated; deadline crossing yields zero click and no post evaluation.
3. Important replacement: exact retained-node disconnection must distinguish MISSING/REPLACED/AMBIGUOUS, none may be VERIFIED, and stable semantic identity must survive unrelated ordinal reorder without confusing a different candidate.
4. Minor download proof: data-event cancellation and admitted HTTP `/__download` zero-delivery must be separate and non-vacuous.

Confirm S03–S08, Task 5 authority/regressions, status precedence, ownership close, immutable bounded data, and evidence-only architecture remain intact. Inspect RED/GREEN tests for non-vacuity. PASS requires no Critical/Important; list Minors separately. Read-only only: no edits, Git, dependencies, live target, or subagent.

## Verification evidence

- Focused Task 11: 47/47 PASS.
...
C024800CB108D579B5310C44C16D9FC066753BDBF670C9BDC3C47074CC6781B2  src/interaction/isolated-auditor.ts
...
1AD2540052B2D7033818797F86D104A4769827FB0D6978178C093922DF44C645  tests/integration/isolated-interaction.test.ts
