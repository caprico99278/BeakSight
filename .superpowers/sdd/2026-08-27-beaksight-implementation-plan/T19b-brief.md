...
## クラウドの環境での追補（2026-09-25 設計者）

このサブタスクは、クラウドの Linux の環境で行います。次のとおり、上の指示を読み替えてください。

- この環境の Node は v22.22.2 です（`package.json` の `engines` は 24）。`npm ci` は設計者が済ませました。依存パッケージを、これ以上入れたり変えたりしないでください。
- Playwright 1.62.1 用の Chromium（revision 1234）は、ネットワークの設定でダウンロードできず、入っていません。ブラウザを起動するテストと `run` は失敗します。**ブラウザを起動するコマンドは実行しないでください。** Chromium を入れる操作もしないでください。
- 設計者の確認では、型チェックとビルドは PASS しました。テストは、Chromium の実行ファイルがないことが原因の失敗 72 件（34 ファイル）を除き、PASS しました。受け入れ条件の `npm run verify` の PASS は、この環境では確かめられません。報告の「未実行項目」に書いてください。代わりに、`npm run typecheck` と `npm run build` を実行し、ブラウザを使わないテスト（たとえば `npx vitest run tests/unit`）の結果を報告してください。
- 作業3の「幅の走査の結果の記録」は、実際の出力では確かめられません。レポートでの見え方は、コード（Evidence の組み立てとレポートの描画）と、`tests/integration/fixture-full-crawl.test.ts` と `T19a-report.md` の記述で確かめてください。何で確かめたかを報告に書き、実際の出力で確かめていないことを「未確認」と書いてください。
- 作業3の「Windows PowerShell の日本語の表示」は、この環境では確かめられません。出どころは、`doc/design/2026-09-23-beaksight-task-14-17-pipeline-report-cli-design.md` の第7章（853〜877 行付近。Windows で確かめた結果）としてください。
- `node dist/cli/index.js --help` と `validate-config` のような、ブラウザを起動しないコマンドは、実行して確かめてください。
