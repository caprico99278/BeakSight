...
function initiateInvalidation(context: BrowserContext, ledger: SafetyLedger): void {
  void invalidateContext(context, ledger).catch((error: unknown) => {
    ledger.recordInvariantViolation({
...
function beginOverflowInvalidationOnce(guardState: GuardState): void {
  ...
  guardState.overflowInvalidation = invalidateContext(guardState.context, guardState.ledger).finally(() => {
