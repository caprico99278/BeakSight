...
type InstallationState = 'INSTALLING' | 'INSTALLED' | 'INVALIDATED';
type PageGuardStatus = 'INSTALLING' | 'READY' | 'FAILED';
type GuardMode = 'PASSIVE' | 'INTERACTION_FROZEN';
...
const installationStates = new WeakMap<BrowserContext, InstallationState>();
const guardStates = new WeakMap<BrowserContext, GuardState>();
const invalidatingContexts = new WeakSet<BrowserContext>();
const ownerClosingContexts = new WeakSet<BrowserContext>();
...
async function invalidateContext(context: BrowserContext, ledger: SafetyLedger): Promise<void> {
  if (
    invalidatingContexts.has(context)
    || installationStates.get(context) === 'INVALIDATED'
  ) {
    return;
  }
  invalidatingContexts.add(context);
  installationStates.set(context, 'INVALIDATED');
  const guardState = guardStates.get(context);
  if (guardState !== undefined) {
    guardState.status = 'INVALIDATED';
  }
  try {
    await context.close();
  } catch (error) {
...
function mustFailPausedDocumentForLifecycle(
  context: BrowserContext,
  page: Page,
  guardState: GuardState,
): boolean {
  return guardState.status === 'INVALIDATED'
    || installationStates.get(context) === 'INVALIDATED'
    || ownerClosingContexts.has(context)
...
export async function awaitPassiveRequestGuardReady(page: Page): Promise<void> {
  ...
  const installationState = installationStates.get(context);
  if (installationState === 'INVALIDATED') {
    throw new Error('Passive request guard context was invalidated');
  }
  const guardState = guardStates.get(context);
  if (guardState === undefined || installationState !== 'INSTALLED') {
  ...
  if (guardState.status === 'INVALIDATED' || installationStates.get(context) === 'INVALIDATED') {
    throw new Error('Passive request guard context was invalidated');
  }
  if (page.isClosed()) {
...
function requireInstalledGuardState(context: BrowserContext): GuardState {
  const installationState = installationStates.get(context);
  if (installationState === 'INVALIDATED') {
    throw new Error('Passive request guard context was invalidated');
  }
...
export function assertPassiveRequestGuardActive(context: BrowserContext): void {
  requireInstalledGuardState(context);
}
...
export async function activateInteractionFreeze(page: Page): Promise<void> {
  const context = page.context();
  const guardState = requireInstalledGuardState(context);
  ...
  if (guardState.mode !== 'PASSIVE') {
    return failClosed(`Interaction freeze transition is invalid from ${guardState.mode}`);
  }
  ...
  guardState.mode = 'INTERACTION_FROZEN';
}
...
export async function closePassiveGuardedContext(context: BrowserContext): Promise<void> {
  const guardState = requireInstalledGuardState(context);
  if (ownerClosingContexts.has(context)) {
    throw new Error('Passive guarded context close is already in progress');
  }
  ownerClosingContexts.add(context);
  installationStates.set(context, 'INVALIDATED');
  guardState.status = 'INVALIDATED';
  ...
  try {
    ...
    ownerClosingContexts.delete(context);
  }
  if (closeFailed) {
    throw closeError;
  }
...
export async function installPassiveRequestGuard(
  ...
  const existingState = installationStates.get(context);
  if (existingState !== undefined) {
    const message = `Passive request guard is already installed or invalidated (${existingState})`;
    ledger.recordInvariantViolation({ code: 'GUARD_REPEAT_INSTALLATION', message });
    throw new Error(message);
  }
  installationStates.set(context, 'INSTALLING');
      ...
      const ready = (async (): Promise<PageDocumentGuard> => {
        ...
        session.on('close', () => {
          if (
            ...
            guardState.trackTask(
              invalidateContext(context, ledger),
              'GUARD_CDP_DETACH_INVALIDATION_FAILED',
            );
          }
        });
        session.on('Fetch.requestPaused', (event) => {
          guardState.trackTask((async () => {
            ...
            if (mustFailPausedDocumentForLifecycle(context, page, guardState)) {
            ...
            if (guardState.mode === 'INTERACTION_FROZEN') {
  ...
  const onPage = (page: Page): void => {
    ...
    if (guardState.status === 'INVALIDATED') {
      return;
    }
  ...
  const onRequestFailed = (request: Request): void => {
    ...
    if (isMainFrame === undefined) {
      guardState.trackTask(
        invalidateContext(context, ledger),
        'GUARD_REQUEST_FAILURE_INVALIDATION_FAILED',
      );
      return;
    }
  ...
  installationStates.set(context, 'INSTALLED');
  guardState.status = 'INSTALLED';
}
